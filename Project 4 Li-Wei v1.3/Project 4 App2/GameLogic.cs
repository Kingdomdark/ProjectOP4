...
namespace Project_4_App2
{
    ...
    public class GameLogic : Microsoft.Xna.Framework.Game
    {
        ...
        protected override void LoadContent()  //read top part, which was already created by monogame
        {
            ...
            {
                block.Visit(); //calls the visit method
            }
        ...
        {
            int count = 0; //created so you know will spawn a bomb or a block
            BlocksFactory blocksFactory = new BlocksFactory(); //creates instance of the blocksfactory

            if (GamePad.GetState(PlayerIndex.One).Buttons.Back == ButtonState.Pressed) //if the back button is pressed, exit.
                Exit();
            // TODO: Add your update logic here
            touchCollection = TouchPanel.GetState();//gets the state of the touch inputs
            if (touchCollection.Count > 0)//just a loop
            {
                if (touchCollection[0].State == TouchLocationState.Pressed)//if there is a press, call the method MoveTest (which is still a test)
                {
            ...
            var elapsedtime = gameTime.ElapsedGameTime.Seconds; //gets the elapsed time
            ...
            if (elapsedtime % 5 == 0) //if the elapsed time is divisible by 5,
            {
                if (count < 5) //if the count is smaller than 5, spawn a pointblock
                {
                    ...
                    count = count + 1; //adds 1 to the count, so you will eventually drop a bomb... read further down
                }
